    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-blue-600">{totalSales} ر.س</p>
        </div>
          ...
          <div className="space-y-2">
            {topProducts.map(([productId, data], index) => (
              <div key={productId} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                <span className="font-medium">المنتج #{index + 1}</span>
                <span className="text-blue-600 font-bold">{data.revenue} ر.س</span>
              </div>
            ))}
          </div>
        </div>
